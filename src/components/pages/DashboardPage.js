...
import { getDashboard, getOnlineOrders, updateOnlineOrderStatus } from '../../api';
...
const OL_STATUS = {
  ...
  ready: { color: '#1db97e', icon: '\ud83d\udd14', label: 'Ready' },
};
const OL_NEXT = {
  pending: { label: 'Accept', next: 'confirmed' },
  confirmed: { label: 'Preparing', next: 'preparing' },
  preparing: { label: 'Ready', next: 'ready' },
  ready: { label: 'Delivered', next: 'delivered' },
};
function LiveOnlineOrders({ onNavigate }) {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (d.success) {
        setOrders(d.data.filter(o => !['delivered', 'cancelled', 'payment_failed'].includes(o.status)));
      }
    } catch { }
  ...
  const advance = async (order, next, label) => {
    try {
      const d = await updateOnlineOrderStatus(order.id, { status: next });
      if (d.success) {
    ...
    <div className="card" style={{ padding: 0, marginBottom: 18 }}>
      <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ fontWeight: 800, fontSize: 14 }}>
          \ud83d\udef5 Live Online Orders
          {orders.length > 0 && <span style={{ marginLeft: 8, background: '#e84a5f', color: '#fff', borderRadius: '50%', width: 20, height: 20, fontSize: 11, fontWeight: 900, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>{orders.length}</span>}
        </div>
        <button style={{ background: 'none', border: '1.5px solid var(--border)', borderRadius: 8, padding: '5px 12px', cursor: 'pointer', fontSize: 12, fontWeight: 700 }}
          onClick={() => onNavigate('online_orders')}>View All</button>
      </div>
      {loading ? (
        <div style={{ padding: 32, textAlign: 'center', color: 'var(--ink2)', fontSize: 13 }}>Loading\u2026</div>
      ) : orders.length === 0 ? (
        <div style={{ padding: 32, textAlign: 'center', color: 'var(--ink2)', fontSize: 13 }}>\u2705 No pending online orders</div>
      ) : (
        orders.map(order => {
          const st = OL_STATUS[order.status] || OL_STATUS.pending;
          const next = OL_NEXT[order.status];
          return (
            <div key={order.id} style={{ padding: '12px 18px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ fontSize: 20 }}>{st.icon}</div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                  <span style={{ fontWeight: 800, fontSize: 14 }}>#{order.id}</span>
                  <span style={{ padding: '2px 8px', borderRadius: 12, fontSize: 11, fontWeight: 800, background: st.color + '18', color: st.color }}>{st.label}</span>
                  <span style={{ fontSize: 11, color: 'var(--ink2)', textTransform: 'capitalize' }}>{(order.order_type || '').replace('_', ' ')}</span>
                </div>
                <div style={{ fontSize: 12, color: 'var(--ink2)' }}>
                  {order.customer_name || 'Guest'}{order.customer_phone ? ` \u00b7 ${order.customer_phone}` : ''}
                  {order.delivery_address ? <span style={{ color: '#e23744' }}> \u00b7 \ud83d\udccd {order.delivery_address.substring(0, 40)}\u2026</span> : ''}
                </div>
              </div>
              <div style={{ textAlign: 'right', flexShrink: 0 }}>
                <div style={{ fontWeight: 900, fontSize: 15, color: 'var(--ink)' }}>\u20b9{Number(order.total || 0).toFixed(0)}</div>
                {next && (
                  <button
                    onClick={() => advance(order, next.next, next.label)}
                    style={{ marginTop: 4, padding: '4px 12px', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 6, fontSize: 11, fontWeight: 800, cursor: 'pointer' }}
                  >
                    {next.label} \u2192
                  </button>
                )}
    ...
    <div>
      ...
      <div className="ph" style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <div style={{
            display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 20, fontSize: 12, fontWeight: 700,
      ...
      {/* Live Online Orders */}
      <LiveOnlineOrders onNavigate={onNavigate || (() => { })} />
