...
// ── Customers (public ordering) ───────────────────────────
export const verifyCustomerPhone = (d) => api.post('/customers/verify-phone', d).then(r => r.data);
export const getCustomerMe = (token) => api.get('/customers/me', { headers: { 'x-customer-token': token } }).then(r => r.data);
export const getCustomerOrders = (token) => api.get('/customers/orders', { headers: { 'x-customer-token': token } }).then(r => r.data);
export const getCustomerOrder = (id, token) => api.get(`/customers/orders/${id}`, { headers: { 'x-customer-token': token } }).then(r => r.data);
export const getCustomerAddresses = (token) => api.get('/customers/addresses', { headers: { 'x-customer-token': token } }).then(r => r.data);
export const saveCustomerAddress = (d, token) => api.post('/customers/addresses', d, { headers: { 'x-customer-token': token } }).then(r => r.data);
export const deleteCustomerAddress = (id, token) => api.delete(`/customers/addresses/${id}`, { headers: { 'x-customer-token': token } }).then(r => r.data);

// ── Add-ons (admin) ───────────────────────────────────────
export const getAddonGroups = () => oGet('/addons');
export const createAddonGroup = (d) => oMutate('post', '/addons', d);
export const updateAddonGroup = (id, d) => oMutate('put', `/addons/${id}`, d);
export const deleteAddonGroup = (id) => oMutate('delete', `/addons/${id}`);
export const createAddonItem = (gid, d) => oMutate('post', `/addons/${gid}/items`, d);
export const updateAddonItem = (iid, d) => oMutate('put', `/addons/items/${iid}`, d);
export const deleteAddonItem = (iid) => oMutate('delete', `/addons/items/${iid}`);
export const getMenuItemAddonGroups = (mid) => oGet(`/addons/menuitem/${mid}`);
export const linkMenuItemAddons = (mid, ids) => oMutate('post', `/addons/menuitem/${mid}/link`, { group_ids: ids });
export const getAllAddonLinks = () => oGet('/addons/all-links');

// ── Public addon groups (no auth) ─────────────────────────
export const getPublicAddonGroups = () => api.get('/public/addon-groups').then(r => r.data);
export const getPublicConfig = () => api.get('/settings/public').then(r => r.data);
export const validatePublicCoupon = (d) => api.post('/public/coupon/validate', d).then(r => r.data);

// ── Online orders ─────────────────────────────────────────
export const placeOnlineOrder = (d) => api.post('/online-orders', d).then(r => r.data);
export const getOnlineOrder = (id) => api.get(`/online-orders/${id}`).then(r => r.data);
export const getOnlineOrders = () => oGet('/online-orders');
export const updateOnlineOrderStatus = (id, d) => oMutate('patch', `/online-orders/${id}/status`, d);

// ── PhonePe ───────────────────────────────────────────────
export const initPhonePePayment = (d) => api.post('/phonepay/initiate', d).then(r => r.data);
export const checkPhonePeStatus = (txnId) => api.get(`/phonepay/status/${txnId}`).then(r => r.data);
export const confirmCODOrder = (d) => api.post('/phonepay/confirm-cod', d).then(r => r.data);
